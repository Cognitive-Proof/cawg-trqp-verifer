...
describe("Verifier", () => {
  it("verifies a standard online request", () => {
  ...
  it("rejects on failed process proof despite valid authorization", () => {
  ...
  it("supports the real C2PA manifest fixture format", () => {
  ...
  it("applies a revocation delta to reject a previously-authorized entity", () => {
    const req = loadManifestFixture("examples/fixtures/cawg_manifest_minimal.json", "did:web:media-registry.example");
    const verifier = new Verifier({ service: new MockTRQPService("data/policies.json", "data/revocations.json") });
    ...
    expect(result.explanations.some((e) => e.includes("revoked_in_epoch_2026-Q2"))).toBe(true);
  });
});
