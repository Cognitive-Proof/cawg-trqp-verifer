...
const fakePool = {
  query: vi.fn(async (sql: string) => {
    if (sql.includes("FROM revocations")) return [revocationRows];
    if (sql.includes("FROM authorizations")) return [authorizationRows];
    if (sql.includes("FROM recognitions")) return [recognitionRows];
    return [[]];
  }),
};

vi.mock("../client.js", () => ({
  getMySQLPool: vi.fn(() => fakePool),
}));

const { MySQLPolicyService } = await import("../index.js");

describe("MySQLPolicyService", () => {
  beforeEach(() => {
    authorizationRows = [];
    recognitionRows = [];
    revocationRows = [];
  ...
  it("authorization() rejects a revoked entity without querying the authorizations table", async () => {
    revocationRows = [{ revoked_entities: JSON.stringify(["did:web:blocked.example"]), policy_epoch: "2026-Q1" }];
    const service = new MySQLPolicyService();
    ...
    authorizationRows = [
      {
        ...
        context: JSON.stringify({ jurisdiction: "IN" }),
        authorized: 1,
        policy_epoch: "2026-Q1",
        evidence: JSON.stringify(["policy:1"]),
        policy_requirements: JSON.stringify({}),
      },
    ];
    const service = new MySQLPolicyService();
